...
import { ignoreSplashGame, setErrorMessage, setSplashGame, setSplashGames, resetDabiGameData } from "../reducer/settingsSlice";
import { store } from "../store";
import { GameIconData } from "./model/GameIconData";
 ...
 */
    ...
    public static moneyFormat(num: any, decimal = 2, split = ',') {
        /*
          parameter：
          num：格式化目标数字
          decimal：保留几位小数，默认2位
        ...
        */
        ...
        const thousandFormat = (num: any): any => {
    ...
    public static showToast(message: string, canClose: boolean = true) {

        store.dispatch(setErrorMessage({ message, canClose, success: false }));

    }

    public static showSuccessToast(message: string) {

        store.dispatch(setErrorMessage({ message, success: true, canClose: true }));

    }



    public static ignoreSplashGame() {
        store.dispatch(ignoreSplashGame({}));
    }

    public static setDabiGameData(option: any) {
        // setDa
        store.dispatch(resetDabiGameData(option));
    }

    public static setUserPlayerInfo(info: any) {
        store.dispatch(setUserInfo(info));
    }

    public static setSplashGames(games: Array<GameIconData>) {
        store.dispatch(setSplashGames({ games }));
    }

    public static closeSplashGame(checkDay: boolean) {
        ...
        store.dispatch(setSplashGame({ today: checkDay }));
        // setSplashGame
